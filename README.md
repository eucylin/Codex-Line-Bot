...
當 webhook 執行失敗時，Apps Script 後台只會顯示「失敗」狀態，可以透過下列方式查看詳細錯誤：

1. 在 Apps Script 編輯器左側選擇 **Executions（執行紀錄）**。
2. 點選失敗的執行紀錄，即可看到錯誤訊息、堆疊資訊以及本專案額外紀錄的 webhook payload。
3. 若要即時觀察錯誤，亦可在編輯器中開啟 **View → Logs**，或在代碼中使用 `Logger.log` / `console.error`（已於 `doPost` 中加入統一的錯誤記錄）。

常見錯誤原因：

- **缺少 Script Properties**：請確認已設定 `SPREADSHEET_ID` 與 `LINE_ACCESS_TOKEN`。
- **權限不足**：首次部署後需按提示授權 Apps Script 使用 Spreadsheet 與 UrlFetch 服務。
