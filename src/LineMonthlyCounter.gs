...
function doPost(e) {
  try {
    ...
    try {
      ...
      logError_('Failed to parse payload', error, e);
    ...
    logError_('Unexpected error in doPost', error, e);
    return createResponse_(500, 'Internal error');
  }
}
...
function logError_(message, error, event) {
  const details = error && error.stack ? error.stack : String(error);
  const payload = event && event.postData && event.postData.contents ? event.postData.contents : '';
  const logMessage = [message, details, payload ? 'Payload: ' + payload : ''].filter(Boolean).join('\n');
